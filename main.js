...
{
	if (region.X.C === region.ab.C)
	{
		return 'on line ' + region.X.C;
	}
	return 'on lines ' + region.X.C + ' through ' + region.ab.C;
}
...
var $author$project$Main$init = {B: $elm$core$Maybe$Nothing, s: 0, D: $elm$core$Set$empty, y: 1};
		...
		switch (msg.$) {
			...
			case 2:
				...
				return _Utils_update(
					model,
					{
						D: A2($elm$core$Set$insert, paperId, model.D)
					});
			case 3:
				var lang = msg.a;
				return _Utils_update(
...
var $author$project$Main$getStrings = function (lang) {
	if (!lang) {
		return {I: 'JST Support for Pioneering Research Initiated by the Next Generation (SPRING)', J: ', Apr. 2025 – Mar. 2026', K: 'French Government Scholarship (France Excellence Japon)', L: ', Sep. 2019 – Sep. 2021', M: 'Fellowships and Scholarships', N: 'CSS: ', O: 'Written in ', P: ' (Oral Presentation)', Q: 'A learning-based homotopy-aware path planning method for a tethered mobile robot to avoid cable-obstacles and cable-robot contacts while navigating to goals.', R: 'A path refinement method for a tethered mobile robot to avoid cable-obstacle contact by considering path curvature and distance from cable base.', S: 'Publications'};
	} else {
		return {I: 'JST次世代研究者挑戦的研究プログラム（SPRING）', J: '、2025年4月 – 2026年3月', K: 'フランス政府奨学金（フランス・エクセレンス・ジャポン）', L: '、2019年9月 – 2021年9月', M: '奨学金・フェローシップ', N: 'CSS参考: ', O: '作成言語: ', P: '（口頭発表）', Q: 'テザー付き移動ロボットがゴールへ向かう際に、ケーブルと障害物の接触およびケーブルとロボット本体の接触を回避するための、学習ベースのホモトピー考慮型経路計画手法。', R: '経路の曲率とケーブル基点からの距離を考慮することで、テザー付き移動ロボットのケーブルと障害物の接触を回避する経路改良手法。', S: '論文'};
	}
};
...
var $author$project$Main$viewPaperImage = F2(
	function (model, paperId) {
		...
		var gifSupported = !A2($elm$core$Set$member, paperId, model.D);
...
var $author$project$Main$publicationsSection = function (model) {
	var s = $author$project$Main$getStrings(model.s);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								$author$project$Main$SetTheme(0))
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('☀')
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Events$onClick(
								$author$project$Main$SetTheme(1))
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('☽')
			...
			[
				...
				_List_fromArray(
					[
						...
						_Utils_Tuple2('dark', model.y === 1)
					]))
			]),
		_List_fromArray(
			[
				$author$project$Main$toggleBar(model),
				$author$project$Main$headerSection(model.s),
				$author$project$Main$publicationsSection(model),
				$author$project$Main$fellowshipsSection(model.s),
				$author$project$Main$footerNote(model.s)
			]));
};
